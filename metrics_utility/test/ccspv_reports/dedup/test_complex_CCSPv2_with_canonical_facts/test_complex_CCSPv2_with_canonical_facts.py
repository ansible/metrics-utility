...
def validate_managed_nodes(file_path):
    ...
    expected_managed_nodes = {
        ...
        16: {
            ...
            "Canonical Facts": {
                ...
                "ansible_product_serial": [
                    "VMware-56 4d 3a 2f 8c 9b 12 34-56 78 90 ab cd ef 12 34"
                ],
        ...
        17: {
            ...
            "Canonical Facts": {
                ...
                "ansible_port": [443],
                "ansible_product_serial": [
                    "VMware-ab cd ef 12 34 56 78 90-12 34 56 78 90 ab cd ef"
                ],
        ...
        18: {
            ...
            "Canonical Facts": {
                ...
                "ansible_product_serial": [
                    "VMware-12 34 56 78 90 ab cd ef-ab cd ef 12 34 56 78 90"
                ],
        ...
        19: {
            ...
            "Canonical Facts": {
                ...
                "ansible_port": [22],
                "ansible_product_serial": [
                    "VMware-dev-01-02-03-04-05-06-07-08-09-10-11-12"
                ],
                "host_name": ["web04.dev"],
            },
            "Facts": {
        ...
        20: {
            ...
            "Canonical Facts": {
                ...
                "ansible_port": [22],
                "ansible_product_serial": [
                    "VMware-stg-01-02-03-04-05-06-07-08-09-10-11-12"
                ],
    ...
    print(
        f"INFO: Managed nodes validation - comparing {len(actual)} actual vs {len(expected_managed_nodes)} expected entries"
    )
...
def validate_ccsp_summary(file_path):
    ...
    has_all_headers = all(
        any(field in str(first_column.get(i, "")) for i in range(10))
        ...
        if isinstance(col_data, dict):
            for row_idx, value in col_data.items():
                if value in [20, 21, 22, 23]:
                    ...
                    actual["structure"][
                        "total_unique_nodes"
                    ] = value  # Use the actual value found
                    break
...
def validate_indirectly_managed_nodes(file_path):
    ...
    for entry_id, expected_entry in expected_indirectly_managed_nodes.items():
        assert (
            entry_id in actual
        ), f"Entry {entry_id} missing from indirectly managed nodes output"
        actual_entry = actual[entry_id]

        for field, expected_value in expected_entry.items():
            assert (
                field in actual_entry
            ), f'Field "{field}" missing from entry {entry_id}'
            actual_value = actual_entry[field]
            assert (
                actual_value == expected_value
            ), f'Entry {entry_id}, field "{field}": expected {expected_value!r}, got {actual_value!r}'
...
def validate_data_collection_status(file_path):
    ...
    df_raw = pandas.read_excel(
        file_path, sheet_name="Data collection status", header=None
    )
    ...
    assert (
        second_table_start is not None
    ), "Could not find second table in Data collection status sheet"

    # Parse first table (missing data gaps)
    table1_df = pandas.read_excel(
        file_path, sheet_name="Data collection status", nrows=second_table_start - 1
    )
    ...
    table2_normalized = {
        k: normalize_for_comparison(v) for k, v in table2_actual.items()
    }
    ...
    # Simple assertions
    assert (
        table1_actual == expected_table1
    ), "Table 1 (missing data gaps) does not match expected"
    assert (
        table2_normalized == expected_table2_normalized
    ), "Table 2 (collection status) does not match expected"
    ...
    if main_host_path.exists():
        ...
        for _, row in df.iterrows():
            try:
                ...
                ), "canonical_facts must be a dict"

                # Validate expected fields
                expected_fields = ["ansible_host", "host_name"]
                for field in expected_fields:
                    assert (
                        field in canonical_facts
                    ), f"Missing {field} in canonical_facts"

            except json.JSONDecodeError:
                pytest.fail(
                    f'Invalid JSON in canonical_facts for row {row["host_name"]}'
                )
        ...
        for _, row in df.iterrows():
            try:
                ...
                if virtualization_type == "VirtualPC":  # Windows
                    ...
                    ), f'Windows host {row["host_name"]} should use winrm'
                    assert (
                        canonical_facts.get("ansible_port") == 5985
                    ), f'Windows host {row["host_name"]} should use port 5985'

                elif virtualization_type == "container":  # Kubernetes
                    assert (
                        connection_type == "kubectl"
                    ), f'Container host {row["host_name"]} should use kubectl'

                elif connection_type == "tcp":  # Network devices
                    assert (
                        virtualization_type == "lxc"
                    ), f'TCP connection host {row["host_name"]} should be lxc'
...
def validate_use_cases(actual_managed_nodes):
    ...
    assert (
        app01_cluster["Automated by organizations"] == 3
    ), f'app01.cluster should show 3 orgs, got {app01_cluster["Automated by organizations"]}'
    cf = get_canonical_facts(app01_cluster)
    assert cf.get("ansible_machine_id") == [
        "e56eb592febecd4e03860514ce5a9f55"
    ], "app01.cluster should have machine_id 'e56eb592febecd4e03860514ce5a9f55'"
    assert cf.get("ansible_product_serial") == [
        "USE1234567"
    ], "app01.cluster should have serial 'USE1234567'"
    ...
    assert (
        "web01.prod.company.com" in hostnames
    ), "web01.prod.company.com should be in host names"
    assert cf.get("ansible_machine_id") == [
        "3a2f8c9b123456789012345678901234"
    ], "Should have correct machine_id"
    ...
    assert cf.get("ansible_machine_id") == [
        "f3e2da65c5d34e59151db7ec18b868d9"
    ], "Should have correct machine_id"
    hostnames = cf.get("host_name", [])
    assert (
        "web02.external" in hostnames and "web02.internal" in hostnames
    ...
    assert cf.get("ansible_machine_id") == [
        "eddfa033379afb7784abb2e4c7dc2cf1"
    ], "Should have correct machine_id"
    hostnames = cf.get("host_name", [])
    assert (
        "db02.dev" in hostnames and "db02.staging" in hostnames
    ), "Should show both hostnames"
    ...
    assert cf.get("ansible_machine_id") == [
        "01b6b28643a6a867e339e957c8ed9d37"
    ], "Should have correct machine_id"
    ...
    assert (
        cache01["Automated by organizations"] == 2
    ), f'cache01.internal should show 2 orgs, got {cache01["Automated by organizations"]}'
    cf = get_canonical_facts(cache01)
    assert cf.get("ansible_machine_id") == [
        "0267fc0887de14e8c994d1025a445221"
    ], "Should have machine_id"
    assert (
        cf.get("ansible_product_serial") is None
        or cf.get("ansible_product_serial") == []
    ), "Should have no serial"
    ...
    assert (
        cf.get("ansible_machine_id") is None or cf.get("ansible_machine_id") == []
    ), "Should have no machine_id"
    ...
    assert not cf.get("ansible_machine_id") and not cf.get(
        "ansible_product_serial"
    ), "Should have no canonical facts"
    ...
    assert (
        web04_dev != web04_staging
    ), "web04.dev and web04.staging should be separate entries"
    ...
    secure_hosts = [
        entry
        for entry in actual_managed_nodes.values()
        if "secure-host-01" in entry["Host name"]
    ]
    # Should have 2 entries due to different privilege levels
    assert (
        len(secure_hosts) == 2
    ), f"Should have 2 secure-host-01 entry (correctly deduplicated), got {len(secure_hosts)}"

    # Test Case 3.4: app01.failover (should be separate)
    app01_failover = find_host("app01.failover")
    assert (
        app01_failover is not None
    ), "app01.failover should be present as separate host"
    cf = get_canonical_facts(app01_failover)
    assert cf.get("ansible_machine_id") == [
        "1a17f31cc8a19e2e1d3aa4901cb47939"
    ], "Should have different machine_id"
    ...
    assert (
        "aws-vm-02.us-west" in hostnames
    ), "AWS VMs should be wrongly merged (expected false positive)"
    ...
    assert (
        "nat-host-01.external" in hostnames and "nat-host-02.external" in hostnames
    ), "NAT hosts should be wrongly merged"
    ...
    assert any(
        "api-server.company.com" in h for h in hostnames
    ), "Should include FQDN variants"
    ...
    db_primary_count = sum(
        1 for h in [db_primary_short, db_primary_fqdn, db_primary_west] if h is not None
    )
    assert (
        db_primary_count == 3
    ), f"Should have 3 separate db-primary entries (false negative), got {db_primary_count}"
