...
def generate_renewal_guidance_dataframe(is_empty=False):
    ...
    now_utc = datetime.now(timezone.utc).replace(microsecond=0) # Current time for reference
    ephemeral_threshold_dt = now_utc - timedelta(days=30)
    iso_deleted_date = (now_utc - timedelta(days=5)).isoformat(timespec='seconds')


    rows = []

    # --- Existing Rows (adjusted for consistency and explicit values) ---
    ...
    rows.append({
        'host_id': 1002, 'hostname': hostname_2,
        'first_automation': first_auto_dt_2.isoformat(timespec='seconds'),
        'last_automation': last_auto_dt_2.isoformat(timespec='seconds'),
        'automated_counter': 730, 'deleted_counter': 0, 'last_deleted': None, 'deleted': False,
        'ansible_product_serial': 'SN' + str(random.randint(10**9, 10**10 - 1)),
        'ansible_machine_id': str(uuid.uuid4()),
        'ansible_host_variable': 'localhost2', 'ansible_connection_variable': 'ssh',
        'days_automated': (last_auto_dt_2 - first_auto_dt_2).days # <-- ADDED THIS FIELD
    })
    ...
    rows.append({
        'host_id': 1003, 'hostname': hostname_3,
        'first_automation': first_auto_dt_3.isoformat(timespec='seconds'),
        'last_automation': last_auto_dt_3.isoformat(timespec='seconds'),
        'automated_counter': 730, 'deleted_counter': 0, 'last_deleted': None, 'deleted': False,
        'ansible_product_serial': 'SN' + str(random.randint(10**9, 10**10 - 1)),
        'ansible_machine_id': str(uuid.uuid4()),
        'ansible_host_variable': 'localhost3', 'ansible_connection_variable': 'winrm',
        'days_automated': (last_auto_dt_3 - first_auto_dt_3).days # <-- ADDED THIS FIELD
    })
    ...
    rows.append({
        'host_id': 1004, 'hostname': hostname_4,
        'first_automation': first_auto_dt_4.isoformat(timespec='seconds'),
        'last_automation': last_auto_dt_4.isoformat(timespec='seconds'),
        'automated_counter': 730, 'deleted_counter': 1, 'last_deleted': iso_deleted_date, 'deleted': True,
        'ansible_product_serial': 'SN' + str(random.randint(10**9, 10**10 - 1)),
        'ansible_machine_id': str(uuid.uuid4()),
        'ansible_host_variable': 'localhost-duplicate', 'ansible_connection_variable': 'ssh',
        'days_automated': (last_auto_dt_4 - first_auto_dt_4).days # <-- ADDED THIS FIELD
    })
    ...
    rows.append({
        'host_id': 1005, 'hostname': hostname_5,
        'first_automation': first_auto_dt_5.isoformat(timespec='seconds'),
        'last_automation': last_auto_dt_5.isoformat(timespec='seconds'),
        'automated_counter': 10, 'deleted_counter': 0, 'last_deleted': None, 'deleted': False,
        'ansible_product_serial': 'SN' + str(random.randint(10**9, 10**10 - 1)),
        'ansible_machine_id': str(uuid.uuid4()),
        'ansible_host_variable': 'ephemeral-host-1', 'ansible_connection_variable': 'ssh',
        'days_automated': (last_auto_dt_5 - first_auto_dt_5).days # <-- ADDED THIS FIELD
    })
    ...
    rows.append({
        'host_id': 1006, 'hostname': hostname_6,
        'first_automation': first_auto_dt_6.isoformat(timespec='seconds'),
        'last_automation': last_auto_dt_6.isoformat(timespec='seconds'),
        'automated_counter': 500, 'deleted_counter': 0, 'last_deleted': None, 'deleted': False,
        'ansible_product_serial': 'SN' + str(random.randint(10**9, 10**10 - 1)),
        'ansible_machine_id': str(uuid.uuid4()),
        'ansible_host_variable': 'stable-host-1', 'ansible_connection_variable': 'ssh',
        'days_automated': (last_auto_dt_6 - first_auto_dt_6).days # <-- ADDED THIS FIELD
    })
    ...
    rows.append({
        'host_id': 1007, 'hostname': hostname_7,
        'first_automation': first_auto_dt_7.isoformat(timespec='seconds'),
        'last_automation': last_auto_dt_7.isoformat(timespec='seconds'),
        'automated_counter': 50, 'deleted_counter': 0, 'last_deleted': None, 'deleted': False,
        'ansible_product_serial': 'SN' + str(random.randint(10**9, 10**10 - 1)),
        'ansible_machine_id': str(uuid.uuid4()),
        'ansible_host_variable': 'ephemeral-boundary-1', 'ansible_connection_variable': 'ssh',
        'days_automated': (last_auto_dt_7 - first_auto_dt_7).days # <-- ADDED THIS FIELD
    })
    ...
    rows.append({
        'host_id': 1008, 'hostname': hostname_8,
        'first_automation': first_auto_dt_8.isoformat(timespec='seconds'),
        'last_automation': last_auto_dt_8.isoformat(timespec='seconds'),
        'automated_counter': 10, 'deleted_counter': 0, 'last_deleted': None, 'deleted': False,
        'ansible_product_serial': 'SN' + str(random.randint(10**9, 10**10 - 1)),
        'ansible_machine_id': str(uuid.uuid4()),
        'ansible_host_variable': 'ephemeral-boundary-2', 'ansible_connection_variable': 'ssh',
        'days_automated': (last_auto_dt_8 - first_auto_dt_8).days # <-- ADDED THIS FIELD
    })
    ...
    rows.append({
        'host_id': 1009, 'hostname': hostname_9,
        'first_automation': first_auto_dt_9.isoformat(timespec='seconds'),
        'last_automation': last_auto_dt_9.isoformat(timespec='seconds'),
        'automated_counter': 200, 'deleted_counter': 1, 'last_deleted': iso_deleted_date, 'deleted': True,
        'ansible_product_serial': 'SN' + str(random.randint(10**9, 10**10 - 1)),
        'ansible_machine_id': str(uuid.uuid4()),
        'ansible_host_variable': 'deleted-host-1', 'ansible_connection_variable': 'ssh',
        'days_automated': (last_auto_dt_9 - first_auto_dt_9).days # <-- ADDED THIS FIELD
    })
    ...
    rows.append({
        'host_id': 1010, 'hostname': hostname_10,
        'first_automation': first_auto_dt_10.isoformat(timespec='seconds'),
        'last_automation': last_auto_dt_10.isoformat(timespec='seconds'),
        'automated_counter': 5, 'deleted_counter': 1, 'last_deleted': iso_deleted_date, 'deleted': True,
        'ansible_product_serial': 'SN' + str(random.randint(10**9, 10**10 - 1)),
        'ansible_machine_id': str(uuid.uuid4()),
        'ansible_host_variable': 'deleted-ephemeral-host-1', 'ansible_connection_variable': 'ssh',
    ...
    def df_managed_nodes_query(self, dataframe, ephemeral=None, with_deleted=False):
        ...
        if ephemeral is None:
            ...
            ephemeral_threshold = pd.to_datetime(
                ...
                ) - timedelta(days=ephemeral_days - 1),
                utc=True
...
def test_managed_nodes_query_ephemeral_true_no_deleted():
    ...
    # Hosts that are EPHEMERAL according to your mock data comments (days_automated <= 30 AND first_automation <= now-30d):
...
def test_managed_nodes_query_ephemeral_false_with_deleted():
    ...
    # Non-ephemeral hosts (from mock data comments):
