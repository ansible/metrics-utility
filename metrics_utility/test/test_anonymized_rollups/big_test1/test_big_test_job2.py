"""
Test for big_test1 - Job 2 scenario based on job2.py description.
...
from metrics_utility.test.test_anonymized_rollups.big_test1.job2 import events, jobhostsummary, jobs
...
def test_big_test2():
    ...
    try:
        ...
        # Filter credentials for job 2 only
        job2_credentials = [c for c in credentials if c['job_id'] == 2]
        pd.DataFrame(job2_credentials).to_csv(credentials_csv, index=False)
        ...
        # Verify credentials (Job 2 has Machine and Vault)
        ...
        assert 'rollup_period_credential_type_vault_total' in statistics
        assert statistics['rollup_period_credential_type_vault_total'] == 1
