"""
Test for big_test1 - Job 4 scenario based on job4.py description.
...
- Host2: Task 1 ok, Task 2 failed (1st attempt) -> failed (2nd attempt) -> ok (retry successful on 3rd attempt), Task 3 ok (successful)
- Host3: Task 1 dark (unreachable), Task 2 ok, Task 3 ok (unreachable - has dark task)
...
from metrics_utility.test.test_anonymized_rollups.big_test1.job4 import events, jobhostsummary, jobs


def test_big_test4():
    """Test anonymized rollups for Job 4 scenario."""
    # Create temporary directory for CSV files
    test_dir = '/tmp/test_big_test4'
    ...
    try:
        ...
        # Filter credentials for job 4 only
        job4_credentials = [c for c in credentials if c['job_id'] == 4]
        pd.DataFrame(job4_credentials).to_csv(credentials_csv, index=False)
        ...
        assert job_type_data['ok_total'] == 10, f'Should have 10 ok tasks total, got {job_type_data["ok_total"]}'
        ...
        # Verify credentials (Job 4 has Machine and Source Control)
        ...
        assert 'rollup_period_credential_type_source_control_total' in statistics
        assert statistics['rollup_period_credential_type_source_control_total'] == 1
