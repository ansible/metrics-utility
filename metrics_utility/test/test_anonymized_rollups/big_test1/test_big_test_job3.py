"""
Test for big_test1 - Job 3 scenario based on job3.py description.
...
def test_big_test3():
    """Test anonymized rollups for Job 3 scenario."""
    # Create temporary directory for CSV files
    test_dir = '/tmp/test_big_test3'
    ...
    try:
        ...
        # Filter credentials for job 3 only
        job3_credentials = [c for c in credentials if c['job_id'] == 3]
        pd.DataFrame(job3_credentials).to_csv(credentials_csv, index=False)
        ...
        assert 'rollup_period_credential_type_network_total' in statistics
        assert statistics['rollup_period_credential_type_network_total'] == 1
