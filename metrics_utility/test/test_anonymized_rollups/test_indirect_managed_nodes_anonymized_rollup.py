...
def test_base_excludes_private_collections():
    """base() strips collections not in collections.json from by_collection."""
    rollup = IndirectManagedNodesAnonymizedRollup()
    ...
    data = {
        'groups': {
            ...
            'OrgA||acme.private_collection': {
                'organization_name': 'OrgA',
                'collection': 'acme.private_collection',
                'host_names': ['host2'],
                'host_count': 1,
            },
        },
    ...
    collection_names = [e['collection'] for e in result['json']['by_collection']]
    assert 'cisco.ios' in collection_names
    assert 'acme.private_collection' not in collection_names


def test_base_excludes_private_modules():
    """base() strips modules whose collection prefix is not in collections.json from by_module."""
    rollup = IndirectManagedNodesAnonymizedRollup()
    ...
    data = {
        ...
        'module_groups': {
            ...
            'OrgA||acme.private_collection.some_module': {
                'organization_name': 'OrgA',
                'module': 'acme.private_collection.some_module',
                'host_names': ['host2'],
                'host_count': 1,
            },
        },
    ...
    module_names = [e['module'] for e in result['json']['by_module']]
    assert 'cisco.ios.ios_command' in module_names
    assert 'acme.private_collection.some_module' not in module_names


def test_base_excludes_no_collection_sentinel():
    """base() excludes the _no_collection sentinel from by_collection."""
    rollup = IndirectManagedNodesAnonymizedRollup()
    ...
    assert result['json']['by_collection'] == []


def test_base_excludes_no_module_sentinel():
    """base() excludes the _no_module sentinel from by_module."""
    rollup = IndirectManagedNodesAnonymizedRollup()
