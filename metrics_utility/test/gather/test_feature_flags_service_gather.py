...
def test_feature_flags_service_command():
    ...
    assert 'FEATURE_SOME_DISABLED_FLAG' not in df['name'].values, (
        'Disabled flag FEATURE_SOME_DISABLED_FLAG should be filtered out'
    )

    # The two enabled flags from test data should be present
    assert 'FEATURE_INDIRECT_NODE_COUNTING_ENABLED' in df['name'].values, (
        'FEATURE_INDIRECT_NODE_COUNTING_ENABLED should be in results'
    )
    assert 'FEATURE_ANALYTICS_ENABLED' in df['name'].values, (
        'FEATURE_ANALYTICS_ENABLED should be in results'
    )
