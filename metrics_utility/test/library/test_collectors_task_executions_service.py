...
def test_task_executions_service_calls_copy_table(mock_copy_pandas):
    ...
    mock_copy_pandas.return_value = pd.DataFrame(
        {
            'started_at': [datetime(2025, 6, 13, 1, 0, 0, tzinfo=UTC)],
            'completed_at': [datetime(2025, 6, 13, 1, 0, 5, tzinfo=UTC)],
...
@pytest.mark.parametrize(
    ...
    [
        (
            datetime(2025, 6, 13, 0, 0, 0, tzinfo=UTC),
            datetime(2025, 6, 14, 0, 0, 0, tzinfo=UTC),
        ),
    ],
)
@patch('metrics_utility.library.collectors.util._copy_table_pandas')
def test_task_executions_service_returns_dataframe_with_expected_columns(mock_copy_pandas, since, until):
    ...
    mock_copy_pandas.return_value = pd.DataFrame(
        {
            'started_at': [
                datetime(2025, 6, 13, 1, 0, 0, tzinfo=UTC),
                datetime(2025, 6, 13, 2, 0, 0, tzinfo=UTC),
                datetime(2025, 6, 13, 3, 0, 0, tzinfo=UTC),
            ],
            'completed_at': [
                datetime(2025, 6, 13, 1, 0, 4, tzinfo=UTC),
                datetime(2025, 6, 13, 2, 0, 5, tzinfo=UTC),
                None,
            ],
    ...
    mock_copy_pandas.return_value = pd.DataFrame(
        {
            ...
            'completed_at': [
                datetime(2025, 6, 13, 1, 0, 4, tzinfo=UTC),
                datetime(2025, 6, 13, 2, 0, 3, tzinfo=UTC),
                datetime(2025, 6, 13, 3, 0, 1, tzinfo=UTC),
            ],
            'collector_type': [
                'unified_jobs',  # hourly
