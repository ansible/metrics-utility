...
def test_parse_date_param():
    ...
    assert parse_date_param('2024-01-01') == datetime.datetime(2024, 1, 1, 0, 0, tzinfo=datetime.UTC)
    assert parse_date_param('2024-01-01T12:31:00Z') == datetime.datetime(2024, 1, 1, 12, 31, tzinfo=datetime.UTC)
    ...
    with patch('metrics_utility.management.validation.now') as mock:
        ...
        assert parse_date_param('1d') == datetime.datetime(2024, 2, 29, 0, 0, tzinfo=datetime.UTC)
        assert parse_date_param('2mo') == datetime.datetime(2023, 12, 29, 0, 0, tzinfo=datetime.UTC)
        assert parse_date_param('3m') == datetime.datetime(2024, 2, 29, 13, 56, tzinfo=datetime.UTC)

    # ensure timezone
    assert parse_date_param('2mo').tzinfo == datetime.UTC
    assert parse_date_param('3m').tzinfo == datetime.UTC
    assert parse_date_param('2024-01-01').tzinfo == datetime.UTC
