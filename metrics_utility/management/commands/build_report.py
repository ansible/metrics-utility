...
class Command(BaseCommand):
        ...
        base.update(
            {
                ...
                "report_sku_description": os.getenv(
                ...
                "report_company_business_leader": os.getenv(
                    "METRICS_UTILITY_REPORT_COMPANY_BUSINESS_LEADER", ""
                ),
                "report_company_procurement_leader": os.getenv(
                    "METRICS_UTILITY_REPORT_COMPANY_PROCUREMENT_LEADER", ""
                ),
                "report_end_user_company_name": os.getenv(
                    "METRICS_UTILITY_REPORT_END_USER_COMPANY_NAME", ""
                ),
                "report_end_user_company_city": os.getenv(
                    "METRICS_UTILITY_REPORT_END_USER_CITY", ""
                ),
                "report_end_user_company_state": os.getenv(
                    "METRICS_UTILITY_REPORT_END_USER_STATE", ""
                ),
                "report_end_user_company_country": os.getenv(
                    "METRICS_UTILITY_REPORT_END_USER_COUNTRY", ""
                ),
                # Renewal guidance specific params
                "report_renewal_guidance_dedup_iterations": os.getenv(
                    "REPORT_RENEWAL_GUIDANCE_DEDUP_ITERATIONS", "3"
                ),
                "report_organization_filter": os.getenv(
                    "METRICS_UTILITY_ORGANIZATION_FILTER", None
                ),
                # S3 specific options
                "s3_bucket_name": os.getenv("METRICS_UTILITY_BUCKET_NAME", None),
                "s3_bucket_endpoint": os.getenv(
    ...
    def _handle_month(self, month):
        ...
        if month is not None:
            ...
            beginning_of_the_previous_month = beginning_of_the_month - relativedelta(
                months=1
            )
