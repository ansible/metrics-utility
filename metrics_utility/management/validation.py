...
def handle_directory_ship_target(ship_target):
    ...
    if not ship_path:
        raise MissingRequiredEnvVar(
            "Missing required env variable METRICS_UTILITY_SHIP_PATH, having destination for the generated data"
        )
...
def handle_s3_ship_target(ship_target):
    ...
    if (
        not bucket_name
        or not bucket_endpoint
        or not bucket_access_key
...
def handle_crc_ship_target(ship_target):
    ...
    if billing_provider == "aws":
        ...
        if not billing_account_id:
            raise MissingRequiredEnvVar(
                "Env var: METRICS_UTILITY_BILLING_ACCOUNT_ID, containing  AWS 12 digit customer id needs to be provided."
            )
        billing_provider_params["billing_account_id"] = billing_account_id
    else:
        raise MissingRequiredEnvVar(
            "Uknown METRICS_UTILITY_BILLING_PROVIDER env var, supported values are [aws]."
        )
...
def validate_ship_target(errors):
    ...
    if ship_target and ship_target not in VALID_SHIP_TARGET:
        errors.append(
            f"Invalid METRICS_UTILITY_SHIP_TARGET: {ship_target}. Valid values: "
            f"{', '.join(VALID_SHIP_TARGET)}"
        )
    return ship_target
...
def validate_ship_path(errors, ship_target):
    ...
    if ship_target == "directory" and ship_target:
        if not os.path.isdir(ship_path):
            errors.append(
                f"Invalid METRICS_UTILITY_SHIP_PATH: {ship_path} "
