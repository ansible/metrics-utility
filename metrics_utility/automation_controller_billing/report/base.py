...
class Base:
    ...
    def add_dedup_count_column(self, dataframe, base_col_name, count_col_name):
        ...
        if base_col_name in dataframe.columns:
            dataframe[count_col_name] = self.calculate_dedup_count(
                dataframe[base_col_name]
            )
        return dataframe
    ...
    def handle_dedup_columns_for_scope(self, dataframe, columns, convert_cols):
        ...
        if self.has_dedup_enabled() and "host_names_before_dedup" in dataframe.columns:
            ...
            self.add_dedup_count_column(
                dataframe, "host_names_before_dedup", "host_names_before_dedup_count"
            )
    ...
    def add_dedup_labels_if_needed(self, labels, column_names):
        ...
        if self.has_dedup_enabled():
            labels.update(
                {k: v for k, v in self.DEDUP_COLUMN_LABELS.items() if k in column_names}
            )
        return labels
    ...
    def convert_cell(self, cell):
        ...
        if isinstance(cell, dict):
            new_cell = {
                k: sorted(list(v)) if isinstance(v, set) else v for k, v in cell.items()
            }
        ...
        # If the cell is a list, convert any set elements inside to sorted lists and dump as a JSON string.
            ...
            new_cell = [
                sorted(list(item)) if isinstance(item, set) else item for item in cell
            ]
    ...
    def _fix_event_host_names(self, mapping_dataframe, destination_dataframe):
        ...
        mapping_dataframe["host_composite_id"] = mapping_dataframe.apply(
            concatenate_columns_mapping, axis=1
        )
        ...
        destination_dataframe["host_name"] = destination_dataframe.apply(
            apply_mapping, axis=1
        )
        destination_dataframe["host_composite_id"] = destination_dataframe.apply(
            concatenate_columns_destination, axis=1
        )
    ...
    def _build_data_section_scope(self, current_row, ws, dataframe, mode=None):
        header_font = Font(
            name=self.FONT, size=10, color=self.BLACK_COLOR_HEX, bold=True
        )
        ...
        columns, convert_cols = self.handle_dedup_columns_for_scope(
            ccsp_report_dataframe, columns, convert_cols
        )

        for col in convert_cols:
            if col in ccsp_report_dataframe.columns:
                ccsp_report_dataframe[col] = ccsp_report_dataframe[col].apply(
                    self.convert_cell
                )
        ...
        self.add_dedup_labels_if_needed(
            labels, ["host_names_before_dedup", "host_names_before_dedup_count"]
        )
        ...
        def extract_infra_info(facts):
            ...
            if isinstance(facts, dict):
                ...
                try:
                    ...
                    return {
                        "infra_type": extract_value(
                            facts_dict.get("infra_type", "Unknown")
                        ),
                        "infra_bucket": extract_value(
                            facts_dict.get("infra_bucket", "Unknown")
                        ),
                        "device_type": extract_value(
                            facts_dict.get("device_type", "Unknown")
                        ),
                    }
                except Exception:
                    pass
    ...
    def _create_infrastructure_summary(self, indirect_nodes):
        ...
        summary_df = indirect_nodes.groupby(
            ["infra_type", "infra_bucket", "device_type"], dropna=False
        ).agg(**agg_dict)
        summary_df = summary_df.reset_index()
        summary_df = summary_df.sort_values(
            ["infra_type", "infra_bucket", "device_type"]
        )
    ...
    def _prepare_usage_columns(
        self, managed_node_type, ccsp_report_dataframe, columns, convert_cols
    ):
        ...
        if managed_node_type == "indirect" or (
            managed_node_type == "direct" and self.has_dedup_enabled()
        ):
        ...
        # Handle deduplication columns if enabled
        columns, convert_cols = self.handle_dedup_columns_for_usage(
            ccsp_report_dataframe, columns, convert_cols
        )

        return columns, convert_cols

    def _build_data_section_usage_by_node(
        self, current_row, ws, dataframe, mode=None, managed_node_type=None
    ):
        header_font = Font(
            name=self.FONT, size=10, color=self.BLACK_COLOR_HEX, bold=True
        )
        ...
        columns, convert_cols = self._prepare_usage_columns(
            managed_node_type, ccsp_report_dataframe, columns, convert_cols
        )
