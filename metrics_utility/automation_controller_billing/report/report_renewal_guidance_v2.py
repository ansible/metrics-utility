...
class ReportRenewalGuidanceV2(Base):
    def __init__(self, dataframe, report_period, extra_params):
        ...
        default_column_widths = {1: 40, 2: 20, 3: 15, 4: 15, 5: 15, 6: 20, 7: 15, 8: 30, 9: 20, 10: 20, 11: 20}
    ...
    def _build_data_section_usage_by_org(self, current_row, ws, dataframe):
        ...
        ccsp_report_dataframe = dataframe.groupby('organization_name', dropna=False).agg(
            host_runs_unique=('host_name', 'nunique'), host_runs=('host_name', 'count'), task_runs=('task_runs', 'sum')
        )
        ccsp_report_dataframe = ccsp_report_dataframe.reset_index()
        ccsp_report_dataframe = ccsp_report_dataframe.reindex(columns=['organization_name', 'host_runs_unique', 'host_runs', 'task_runs'])
