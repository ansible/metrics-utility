...
class ReportCCSPv2(Base):
    ...
    def _apply_filter(self, job_host_summary_dataframe, events_dataframe):
        if self.extra_params["report_organization_filter"] is not None:
            ...
            job_host_summary_dataframe = job_host_summary_dataframe[
                job_host_summary_dataframe["organization_name"].isin(org_filter)
            ].copy()
    ...
    def build_spreadsheet(self):
        ...
        events_dataframe = self._fix_event_host_names(
            job_host_summary_dataframe, events_dataframe
        )
        # TODO: also apply organization filter
        job_host_summary_dataframe, events_dataframe = self._apply_filter(
            job_host_summary_dataframe, events_dataframe
        )
        ...
        # First sheet index
        sheet_index = 0
        directs = job_host_summary_dataframe[
            job_host_summary_dataframe["managed_node_type"] == DIRECT
        ]
        indirects = job_host_summary_dataframe[
            job_host_summary_dataframe["managed_node_type"] == INDIRECT
        ]

        if "ccsp_summary" in self.optional_report_sheets():
            ws = self.add_sheet(
                "Usage Reporting", sheet_index, self.config["column_widths"]
            )
        ...
        if "managed_nodes" in self.optional_report_sheets():
            ...
            ws = self.add_sheet(
                "Managed nodes", sheet_index, self.config["data_column_widths"]
            )
        ...
        if "indirectly_managed_nodes" in self.optional_report_sheets():
            ws = self.add_sheet(
                ...
                sheet_index,
                self.config["data_column_widths"],
            )
            ...
            self._build_data_section_usage_by_node(
                1, ws, indirects, managed_node_type="indirect"
            )
            sheet_index += 1

        if "inventory_scope" in self.optional_report_sheets():
            ws = self.add_sheet(
                "Inventory Scope", sheet_index, self.config["data_column_widths"]
            )
        ...
        if "usage_by_organizations" in self.optional_report_sheets():
            ...
            ws = self.add_sheet(
                "Usage by organizations", sheet_index, self.config["data_column_widths"]
            )
        ...
        if events_dataframe is not None:
            ...
            if "usage_by_roles" in self.optional_report_sheets():
                ...
                ws = self.add_sheet(
                    "Usage by roles", sheet_index, self.config["data_column_widths"]
                )
            ...
            if "usage_by_modules" in self.optional_report_sheets():
                ...
                ws = self.add_sheet(
                    "Usage by modules", sheet_index, self.config["data_column_widths"]
                )
        ...
        if "managed_nodes_by_organizations" in self.optional_report_sheets():
            ...
            organization_names = sorted(
                job_host_summary_dataframe["organization_name"].unique()
            )
            for organization_name in organization_names:
                ws = self.add_sheet(
                    organization_name, sheet_index, self.config["data_column_widths"]
                )
                ...
                self._build_data_section_usage_by_node(
                    1, ws, filtered_job_host_summary_dataframe, mode="by_organization"
                )
                sheet_index += 1
        ...
        if "data_collection_status" in self.optional_report_sheets():
            ws = self.add_sheet(
                ...
                sheet_index,
                self.config["status_column_widths"],
            )
            current_row = self._build_data_section_collection_missing(
                1, ws, status_dataframe
            )
            current_row += 1  # gap
            self._build_data_section_collection_status(
                current_row, ws, status_dataframe
            )
            sheet_index += 1
    ...
    def _since_until(self):
        ...
        if self.extra_params["opt_since"]:
            since = self.extra_params["opt_since"].date()
        elif self.extra_params["month_since"]:
            since = (
                self.extra_params["month_since"].date()
                if isinstance(self.extra_params["month_since"], datetime.datetime)
                else self.extra_params["month_since"]
            )

        if self.extra_params["opt_until"]:
            until = self.extra_params["opt_until"].date()
        elif self.extra_params["month_until"]:
            until = (
                self.extra_params["month_until"].date()
                if isinstance(self.extra_params["month_until"], datetime.datetime)
                else self.extra_params["month_until"]
            )
    ...
    def _build_data_section_collection_status(self, first_row, ws, df):
        ...
        df = df.sort_values(["file_name", "collection_start_timestamp"]).reset_index(
            drop=True
        )
    ...
    def _build_data_section_usage_by_node_with_org_details(
        ...
        ccsp_report_dataframe = ccsp_report_dataframe.join(
            pivoted_dataframe, how="left"
        )
        ccsp_report_dataframe = ccsp_report_dataframe.reset_index()
    ...
    def _build_data_section_usage_by_job(self, current_row, ws, dataframe):
        ...
        dataframe["job_remote_id_install_uuid"] = list(
            zip(dataframe["job_remote_id"], dataframe["install_uuid"])
        )

        # Rename the columns based on the template
        ccsp_report_dataframe = dataframe.groupby(
            ["organization_name", "job_template_name"], dropna=False
        ).agg(
    ...
    def _build_data_section_usage_by_org(self, current_row, ws, dataframe):
        ...
        agg_dict = {
            ...
            "host_runs_unique": (
                "host_name",
                lambda x: x[
                    dataframe.loc[x.index, "managed_node_type"] == DIRECT
                ].nunique(),
            ),
            "host_runs": (
                "host_name",
        ...
        if "indirectly_managed_nodes" in self.optional_report_sheets():
            agg_dict["indirect_host_runs_unique"] = (
                "host_name",
                lambda x: x[
                    dataframe.loc[x.index, "managed_node_type"] == INDIRECT
                ].nunique(),
            )
        ...
        ccsp_report_dataframe = dataframe.groupby(
            "organization_name", dropna=False
        ).agg(**agg_dict)
        ...
        if "indirectly_managed_nodes" not in self.optional_report_sheets():
            ccsp_report_dataframe.drop(
                ["indirect_host_runs_unique", "indirect_host_runs"],
                axis=1,
                inplace=True,
            )
    ...
    def _build_header(self, current_row, ws):
        ...
        for header_row in self.config["header"]:
            header_label_font = Font(
                name=self.FONT, size=12, color=self.BLACK_COLOR_HEX
            )
            header_value_font = Font(
                name=self.FONT, size=12, color=self.BLACK_COLOR_HEX
            )
    ...
    def _build_sku_description(self, current_row, ws):
        ...
        for header_row in self.config["sku_description"]:
            header_font = Font(
                name=self.FONT, size=11, color=self.BLACK_COLOR_HEX, bold=True
            )
    ...
    def _build_data_section(self, current_row, ws, dataframe):
        ...
        if quantity_consumed > 0:
            ...
            ccsp_report["end_user_company_name"] = self.extra_params[
                "report_end_user_company_name"
            ]
            ccsp_report["end_user_company_city"] = self.extra_params[
                "report_end_user_company_city"
            ]
            ccsp_report["end_user_company_state"] = self.extra_params[
                "report_end_user_company_state"
            ]
            ccsp_report["end_user_company_country"] = self.extra_params[
                "report_end_user_company_country"
            ]
            ...
            ccsp_report["extended_unit_price"] = round(
                (ccsp_report["quantity_consumed"] * ccsp_report["unit_price"]), 2
            )
                ...
                if row_counter == 0:
                    ...
                    if c_idx >= 8 and c_idx <= 11:
                        cell.fill = PatternFill(
                            "solid", fgColor=self.LIGHT_BLUE_COLOR_HEX
                        )
                    ...
                    if c_idx == 10:
                        ...
                        cell_m_1 = ws.cell(row=r_idx, column=7).column_letter + str(
                            r_idx
                        )
                        cell_m_2 = ws.cell(row=r_idx, column=9).column_letter + str(
                            r_idx
                        )
