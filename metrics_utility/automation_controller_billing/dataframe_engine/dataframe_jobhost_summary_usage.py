...
class DataframeJobhostSummaryUsage(Base):
    ...
    def group(self, dataframe):
        group = dataframe.groupby(self.unique_index_columns(), dropna=False).agg(
            ...
            events=('events', lambda x: merge_arrays([item for item in x if item is not None and not pd.isnull(item)])),
            canonical_facts=('canonical_facts', lambda x: merge_json_sets([item for item in x if item is not None and not pd.isnull(item)])),
            facts=('facts', lambda x: merge_json_sets([item for item in x if item is not None and not pd.isnull(item)])),
