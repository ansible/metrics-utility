...
class DataframeJobhostSummaryUsage(Base):
    ...
    def build_dataframe(self):
        ...
        for date in self.dates():
            for data in self.extractor.iter_batches(date=date):
                ...
                print(f"DEBUG: billing_data head (relevant cols):\n{billing_data[['host_name', 'managed_node_type', DEVICE_TYPE_COL, INFRASTRUCTURE_COL, 'category', 'facts', 'events', 'task_runs']].head().to_markdown(index=False)}")
    ...
    def group(self, dataframe):
        ...
        print(f"\n--- DEBUGGING: group method input for this batch ---")
        print(f"DEBUG: Input dataframe head (for grouping):\n{dataframe[['host_name', 'managed_node_type', DEVICE_TYPE_COL, INFRASTRUCTURE_COL, 'category', 'facts', 'events']].head().to_markdown(index=False)}")
