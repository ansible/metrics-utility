...
def config(since, **kwargs):
    ...
    return {
        ...
        'metrics_utility_version': '0.6.2',  # TODO read from setup.cfg
            ...
            for series in response['data']['result']:
                if 'values' in series:
                    for timestamp_val, cpu_val in series['values']:
                        result.append(
                            {'timestamp': datetime.fromtimestamp(float(timestamp_val), timezone.utc).isoformat(timespec='milliseconds').replace('+00:00', 'Z'), 'cpu_sum': float(cpu_val)}
                        )
