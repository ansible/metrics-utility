...
FILTERS='?is_deprecated=false&is_highest=true&order_by=name'

for REPOSITORY in published validated; do
  COUNT=`curl -H "Authorization: Bearer $BEARER" -s "$API""$FILTERS""&repository_name=$REPOSITORY&offset=0&limit=1" | jq -r .meta.count`
  echo COUNT=$COUNT
  ...
  while [ "$OFFSET" -lt "$COUNT" ]; do
    ...
    curl -H "Authorization: Bearer $BEARER" -s "$API""$FILTERS""&repository_name=$REPOSITORY&offset=$OFFSET&limit=$LIMIT" | jq > hub."$REPOSITORY"."$OFFSET"
    OFFSET=$((OFFSET + LIMIT))
  done
done
