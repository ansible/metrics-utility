#!/usr/bin/env python
"""
Script to collect data from anonymized collectors for a given time range.

Usage:
    python run.py [--since YYYY-MM-DD HH:MM:SS] [--until YYYY-MM-DD HH:MM:SS] [--batches N] [--no-events]
...
Use --no-events to skip the main_jobevent_service collector (events are collected by default).

Output files:

- Final report in ./out/anonymized_rollup.json
...
def main():
    parser = argparse.ArgumentParser(
        description='Collect data from anonymized collectors',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Use default time range (with events)
  python run.py
  ...
  # Custom time range
  python run.py --since "2025-06-13 00:00:00" --until "2025-06-14 00:00:00"

  # Split into 4 batches
  python run.py --since "2025-06-13 00:00:00" --until "2025-06-14 00:00:00" --batches 4

  # Hourly split without events
  python run.py --since "2025-06-13 00:00:00" --until "2025-06-14 00:00:00" --no-events
        """,
    )
    parser.add_argument(
        '--since',
    ...
    parser.add_argument(
        '--no-events',
        action='store_true',
        help='Disable events collection (main_jobevent_service)',
    )
    ...
    print(f'Events collection: {"enabled" if collect_events else "disabled"}')
    print()

    # Get database connection
    db = connection

    # Build the active collectors dict
    active_collectors = dict(COLLECTORS)
    if collect_events:
        active_collectors.update(EVENTS_COLLECTOR)

    # Collect data for each collector
    results: Dict[str, List[pd.DataFrame]] = {}
    collector_times: Dict[str, float] = {}

    for collector_name, collector_info in active_collectors.items():
    ...
    for collector_name, dataframes in results.items():
        ...
        if active_collectors.get(collector_name, {}).get('needs_since_until', False):
    ...
    if not collect_events:
        input_data['main_jobevent'] = []  # Empty when events are disabled
    ...
    # Save final JSON
    json_path = './out/anonymized_rollup.json'
